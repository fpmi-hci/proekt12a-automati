...
public class OrderController {
    ...
    public ResponseEntity<UserOrder> addUserOrder(@RequestBody @Valid UserOrderRequestDto order) {
        UserOrder addedUserOrder = orderService.addOrder(order);
        return new ResponseEntity<>(addedUserOrder, HttpStatus.ACCEPTED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<UserOrder> updateOrder(@RequestBody  @Valid UserOrderRequestDto userOrder, @PathVariable("id") long id){
